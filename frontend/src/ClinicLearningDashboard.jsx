  ...
  return (
    <main className="clinic-course-shell">
      ...
      <section className="clinic-course-main">
        ...
        {section === 'aulas' ? <><header className="clinic-course-header"><span>Videoaulas</span><h1>Aulas do treinamento</h1><p>Elétrica e Hidráulica organizadas por aula.</p></header><div className="clinic-filter-row">{['Todos', 'Elétrica', 'Hidráulica'].map((item) => <button key={item} type="button" className={moduleFilter === item ? 'is-active' : ''} onClick={() => setModuleFilter(item)}>{item}</button>)}</div><div className="clinic-lessons-grid">{visibleLessons.map((lesson) => <article className={`clinic-lesson-card${completed.has(lesson.id) ? ' is-complete' : ''}`} key={lesson.id}><div className="clinic-lesson-copy"><span>{lesson.module}</span><h2>{lesson.id}. {lesson.title}</h2><p>{lesson.description}</p></div>{playingLessonId === lesson.id && lesson.url ? <div className="clinic-lesson-player" style={{ position: 'relative', width: '100%', aspectRatio: '16 / 9', overflow: 'hidden', background: '#000', borderRadius: 18, border: '1px solid rgba(79,225,194,.2)' }}><video src={lesson.url} controls autoPlay playsInline onEnded={() => setPlayingLessonId(null)} style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'contain', border: 0, background: '#000' }} /><button type="button" onClick={() => setPlayingLessonId(null)} aria-label="Fechar vídeo e voltar para a capa" style={{ position: 'absolute', zIndex: 20, top: 10, right: 10, width: 38, height: 38, borderRadius: '50%', border: '1px solid rgba(255,255,255,.45)', background: 'rgba(0,0,0,.78)', color: '#fff', fontSize: 24, lineHeight: 1, display: 'grid', placeItems: 'center', cursor: 'pointer' }}>×</button></div> : <button type="button" className="clinic-video-placeholder clinic-lesson-cover" disabled={!lesson.url} onClick={() => lesson.url && setPlayingLessonId(lesson.id)} aria-label={lesson.url ? `Assistir ${lesson.title}` : `${lesson.title} em preparação`}><img src={CLINIC_LOGO} alt="" aria-hidden="true" /><span className="clinic-lesson-module-badge">{lesson.module}</span><span className="clinic-cover-play" aria-hidden="true">▶</span></button>}<div className="clinic-lesson-actions" style={{ display: 'flex', justifyContent: 'center' }}><button type="button" onClick={() => toggleLesson(lesson.id)} style={{ width: 'auto', minWidth: 220, maxWidth: '82%', padding: '11px 32px' }}>{completed.has(lesson.id) ? '✓ Aula concluída' : 'Aula concluída'}</button></div></article>)}</div></> : null}
